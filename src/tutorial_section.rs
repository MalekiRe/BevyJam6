...
fn do_tutorial(mut commands: Commands) {
	...
	commands.spawn_task(move || async {
        ...
        let (e1, e2, e3, e4) = AsyncWorld.resource_scope(|asset_server: Mut<AssetServer>| {
        ...
        for i in 0..128 {
            ...
            e2.component::<Sprite>().get_mut(|s| s.color.set_alpha(val)).unwrap();
            e1.component::<Sprite>().get_mut(|s| s.color.set_alpha(val)).unwrap();
            AsyncWorld.yield_now().await;
        }

        let e2 = AsyncWorld.spawn_bundle((
            Text2d::new("Your chain must be balanced\n\
        ...
        for i in 0..64 {
            ...
            e3.component::<Sprite>().get_mut(|s| s.color.set_alpha(val)).unwrap();
            e4.component::<Sprite>().get_mut(|s| s.color.set_alpha(val)).unwrap();
            AsyncWorld.yield_now().await;
        }
...
fn settings_menu(_: Trigger<Pointer<Click>>, mut next_menu: ResMut<NextState<GameState>>) {
